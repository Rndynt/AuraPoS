export const ENTITLEMENT_CATALOG = {
  ...
  plans: {
    starter: { label: 'Starter', sortOrder: 10, price: 0, billingInterval: 'monthly', included: ['inventory_basic_stock', 'payments_partial_payment'] },
  ...
  entitlements: {
    inventory_basic_stock: {
      label: 'Stok Dasar', kind: 'module', area: 'inventory', category: 'Inventori',
      description: 'Lihat stok per produk, status menipis/habis, & adjust qty.',
      longDesc: 'Fitur stok esensial: tracking stok per produk, status stok, dan penyesuaian stok dasar.',
    },
    inventory_advanced_stock: {
      label: 'Stok Lanjutan', kind: 'module', area: 'inventory', category: 'Inventori',
      description: 'Mutasi stok bertipe, riwayat audit trail, & laporan pergerakan.',
      longDesc: 'Melengkapi Stok Dasar dengan mutasi bertipe, riwayat audit, opname, transfer, low stock alert, dan laporan stok.',
    ...
    orders_queue: {
      label: 'Antrian Order', kind: 'feature', area: 'orders', category: 'Order',
      description: 'Panel antrian semua order aktif real-time di layar kasir.',
      longDesc: 'Panel yang menampilkan semua order aktif secara real-time beserta status bayar.',
    },
    restaurant_table_service: {
      label: 'Layanan Meja', kind: 'module', area: 'restaurant', category: 'Restoran',
      ...
      bundleItems: [{ label: 'Denah Meja' }, { label: 'Status Meja' }, { label: 'Order per Meja' }],
    },
    restaurant_kitchen_ops: {
      label: 'Kitchen Display (KDS)', kind: 'module', area: 'restaurant', category: 'Restoran',
      description: 'Tiket dapur, layar KDS, & printer dapur dalam satu paket.',
      longDesc: 'Operasional dapur terintegrasi: tiket pesanan ke dapur, layar display staf dapur, dan dukungan printer dapur.',
      bundleItems: [{ label: 'Tiket Dapur' }, { label: 'Layar KDS' }, { label: 'Printer Dapur' }],
    },
    reports_advanced: {
      label: 'Laporan Lanjutan', kind: 'feature', area: 'reports', category: 'Laporan',
      description: 'Dashboard analitik, grafik real-time, & insight penjualan.',
      longDesc: 'Dashboard visual untuk omzet, produk terlaris, nilai transaksi, dan insight bisnis.',
      bundleItems: [{ label: 'Analitik Penjualan' }, { label: 'Performa Kasir' }, { label: 'Ringkasan Bisnis' }],
    },
    reports_export: {
      label: 'Ekspor Laporan', kind: 'feature', area: 'reports', category: 'Laporan',
      description: 'Ekspor laporan ke Excel/PDF untuk akuntansi & arsip.',
      longDesc: 'Ekspor laporan penjualan dan inventori untuk kebutuhan akuntansi, audit, atau arsip.',
    },
    multi_location: {
      label: 'Multi Lokasi', kind: 'module', area: 'multi_location', category: 'Ekspansi',
      description: 'Kelola beberapa cabang dari satu dashboard terpusat.',
      longDesc: 'Kelola beberapa cabang: laporan per cabang, produk/harga per lokasi, dan transfer stok antar cabang.',
      bundleItems: [{ label: 'Cabang' }, { label: 'Stok Cabang' }, { label: 'Laporan Cabang' }],
    },
    hardware_label_printer: {
      label: 'Printer Label', kind: 'feature', area: 'hardware', category: 'Hardware',
    ...
    hardware_barcode_scanner: {
      label: 'Scanner Barcode', kind: 'feature', area: 'hardware', category: 'Hardware',
    ...
    integrations_payment_gateway: {
      label: 'Payment Gateway', kind: 'integration', area: 'integrations', category: 'Integrasi',
      description: 'Terima pembayaran digital dari provider eksternal.',
      longDesc: 'Integrasi payment gateway untuk pembayaran digital dan rekonsiliasi otomatis.',
    },
    integrations_accounting: {
      label: 'Akuntansi', kind: 'integration', area: 'integrations', category: 'Integrasi',
      description: 'Sinkron transaksi ke sistem akuntansi atau Excel.',
      longDesc: 'Sinkron data penjualan dan pembayaran ke software akuntansi atau ekspor otomatis.',
    },
    integrations_webhook: {
      label: 'Webhook', kind: 'integration', area: 'integrations', category: 'Integrasi',
      description: 'Kirim event transaksi otomatis ke sistem eksternal.',
      longDesc: 'Webhook untuk mengirim event order dan pembayaran secara real-time ke sistem eksternal.',
    },
    integrations_api_access: {
      label: 'API Access', kind: 'integration', area: 'integrations', category: 'Integrasi',
      description: 'API key untuk integrasi ke sistem eksternal.',
      longDesc: 'API key dan dokumentasi REST untuk integrasi ERP, marketplace, akuntansi, atau sistem internal.',
    },
  },
  offers: {
    receipt_compact_monthly: { entitlement: 'receipt_compact', requiredPlan: 'starter', price: 15000, billingInterval: 'monthly', expires: true },
    inventory_advanced_stock_addon: { entitlement: 'inventory_advanced_stock', requiredPlan: 'growth', price: 59000, billingInterval: 'monthly', expires: true },
    orders_queue_addon: { entitlement: 'orders_queue', requiredPlan: 'growth', price: 25000, billingInterval: 'monthly', expires: true },
    integrations_webhook_monthly: { entitlement: 'integrations_webhook', requiredPlan: 'growth', price: 49000, billingInterval: 'monthly', expires: true },
  },
  businessTypes: {
    CAFE_RESTAURANT: {
      label: 'Cafe / Restaurant', defaultPlan: 'starter', defaultEntitlements: ['inventory_basic_stock'],
    ...
    RETAIL_MINIMARKET: {
      label: 'Retail / Minimarket', defaultPlan: 'starter', defaultEntitlements: ['inventory_basic_stock'],
    ...
    LAUNDRY: {
      label: 'Laundry', defaultPlan: 'starter', defaultEntitlements: ['inventory_basic_stock'],
    ...
    SERVICE_APPOINTMENT: {
      label: 'Service / Appointment', defaultPlan: 'starter', defaultEntitlements: ['inventory_basic_stock'],
    ...
    DIGITAL_PPOB: {
      label: 'Digital / PPOB', defaultPlan: 'starter', defaultEntitlements: ['inventory_basic_stock'],
