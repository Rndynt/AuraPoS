...
export type LocalPayment = { id: string; localOrderId: string; tenantId: string; amount: number; method: string; idempotencyKey: string; syncStatus: SyncStatus; createdAtLocal: string };
export type LocalPrintJob = { id: string; tenantId: string; terminalId: string; localOrderId?: string; type: "receipt" | "kitchen"; payload: unknown; syncStatus: SyncStatus; status: "pending" | "printing" | "printed" | "failed" | "cancelled"; retryCount: number; createdAt: string; updatedAt: string };
