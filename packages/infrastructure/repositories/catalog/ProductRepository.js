...
import { products, productOptionGroups, productOptions, } from '../../../../shared/schema';
import { eq, and, inArray } from 'drizzle-orm';
/**
 * Map database product to domain product (camelCase -> snake_case)
 */
function mapProductToDomain(dbProduct) {
    return {
        id: dbProduct.id,
 ...
 */
function mapOptionGroupToDomain(dbGroup, options = []) {
    return {
        id: dbGroup.id,
        name: dbGroup.name,
        selection_type: dbGroup.selectionType,
 ...
 */
function mapOptionToDomain(dbOption) {
    return {
        id: dbOption.id,
        name: dbOption.name,
...
export class ProductRepository extends BaseRepository {
     ...
     */
    async findByIdWithOptions(id, tenantId) {
        try {
     ...
     */
    async update(id, product, tenantId) {
        try {
     ...
     */
    async findOptionGroupsByProductIds(productIds) {
        try {
     ...
     */
    async findOptionsByGroupIds(groupIds) {
        try {
     ...
     */
    async findOptionGroupsByProductId(productId) {
        try {
            const groups = await this.db
                .select()
                .from(productOptionGroups)
